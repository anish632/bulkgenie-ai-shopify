...
export function sanitizeGeneratedContent(
  ...
  const appNamePattern = /Bulk\s*Genie(?:\s*AI)?/gi;

  return value
    .replace(
      new RegExp(
        `\\s+(?:by|from|with)\\s+${appNamePattern.source}\\b`,
        "gi",
      ),
      "",
    )
    .replace(
      new RegExp(`\\b${appNamePattern.source}\\s*[-–—:|]\\s*`, "gi"),
      "",
    )
    .replace(
      new RegExp(`\\s*[-–—:|]\\s*${appNamePattern.source}\\b`, "gi"),
      "",
    )
    .replace(new RegExp(`\\b${appNamePattern.source}\\b`, "gi"), "")
    .replace(/\b(?:AI-generated|generated by AI|generated with AI)\b/gi, "")
    .replace(/\s+([,.;:!?])/g, "$1")
    .replace(/\s{2,}/g, " ")
    .replace(/\s+([-–—|])\s+/g, " $1 ")
    .replace(/^\s*[-–—|:]\s*/, "")
